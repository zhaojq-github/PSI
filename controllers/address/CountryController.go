...
func (ctl *AddressCountryController) Validator() {
	...
	if _, arrs, err := md.GetAllAddressCountry(query, exclude, cond, fields, sortby, order, 0, 2); err == nil {
		if len(arrs) == 1 {
...
// 获得符合要求的款式数据
func (ctl *AddressCountryController) addressCountryList(query map[string]interface{}, exclude map[string]interface{}, cond map[string]map[string]interface{}, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {
...
func (ctl *AddressCountryController) PostList() {
	...
	if result, err := ctl.addressCountryList(query, exclude, cond, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
