...
// 获得符合要求的款式数据
func (ctl *AddressDistrictController) addressDistrictList(query map[string]interface{}, exclude map[string]interface{}, cond map[string]map[string]interface{}, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {
...
func (ctl *AddressDistrictController) PostList() {
	...
	if result, err := ctl.addressDistrictList(query, exclude, cond, fields, sortby, order, offset, limit); err == nil {
		ctl.Data["json"] = result
	}
	ctl.ServeJSON()
