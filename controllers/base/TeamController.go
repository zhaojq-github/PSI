...
func (ctl *TeamController) Validator() {
	...
	if _, arrs, err := md.GetAllTeam(query, exclude, cond, fields, sortby, order, 0, 2); err == nil {
		if len(arrs) == 1 {
...
// 获得符合要求的团队数据
func (ctl *TeamController) teamList(query map[string]interface{}, exclude map[string]interface{}, cond map[string]map[string]interface{}, fields []string, sortby []string, order []string, offset int64, limit int64) (map[string]interface{}, error) {
