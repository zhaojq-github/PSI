...
func (ctl *ProductAttributeController) Put() {
	...
	if err = ctl.ParseForm(attribute); err == nil {
		...
		if id, err = md.UpdateProductAttribute(attribute, &ctl.User); err == nil {
			result["code"] = "success"
...
func (ctl *ProductAttributeController) Validator() {
	...
	if _, arrs, err := md.GetAllProductAttribute(query, exclude, cond, fields, sortby, order, 0, 2); err == nil {
		if len(arrs) == 1 {
