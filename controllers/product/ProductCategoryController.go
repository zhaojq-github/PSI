...
func (ctl *ProductCategoryController) Put() {
	...
	productCategory := new(md.ProductCategory)
	var (
		err error
		id  int64
	)
	if err = ctl.ParseForm(productCategory); err == nil {

		// 获得struct表名
		// structName := reflect.Indirect(reflect.ValueOf(company)).Type().Name()
		if id, err = md.UpdateProductCategory(productCategory, &ctl.User); err == nil {
			result["code"] = "success"
...
func (ctl *ProductCategoryController) Validator() {
	...
	if _, arrs, err := md.GetAllProductCategory(query, exclude, cond, fields, sortby, order, 0, 2); err == nil {
		if len(arrs) == 1 {
